...
def patch_wise_robustness_test_second(loader, model, config, eps=2/255, alpha=1/255, iters=250, patch_size=16, num_patches_to_perturb=1):
    ...
    # Commented out adversarial accuracy evaluation
    ...
    #     adv_inputs, save_count = patch_fool_pgd_attack_second(
    #         model, inputs, labels,
    ...
    # log_str += (f"\nPatch-Wise PGD Adversarial Accuracy (eps={eps:.4f}, highest-attention patch): {adv_acc:.2f}%\n"
    #             f"Robustness Drop: {clean_acc - adv_acc:.2f}%")
    print(log_str)
    config["out_file"].write(log_str + "\n")
    config["out_file"].flush()
    
    return clean_acc  # Removed adv_acc from return
