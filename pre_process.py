...
def image_train(resize_size=256, crop_size=224, alexnet=False, ViT=False):
    if not alexnet:
    ...
    if ViT:
        transforms_train = transform_train = transforms.Compose([
            transforms.RandomResizedCrop(resize_size, scale=(0.2, 1.0), interpolation=3),  # 3 is bicubic
...
def image_test(resize_size=256, crop_size=224, alexnet=False, ViT=False):
    if not alexnet:
        normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                         std=[0.229, 0.224, 0.225])
    else:
...
def image_test_10crop(resize_size=256, crop_size=224, alexnet=False, ViT=False):
    if not alexnet:
        normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                         std=[0.229, 0.224, 0.225])
    else:
