...
def vit_base_patch16_224(pretrained=False, progress=True, **kwargs):
    model = timm.create_model('vit_base_patch16_224', pretrained=pretrained, **kwargs)
    return model


def vit_large_patch16_224(pretrained=False, progress=True, **kwargs):
    model = timm.create_model('vit_large_patch16_224', pretrained=pretrained, **kwargs)
    return model


def vit_huge_patch16_224(pretrained=False, progress=True, **kwargs):
    model = timm.create_model('vit_huge_patch14_224', pretrained=pretrained, **kwargs)
    return model


vit_dict = {"vit_small_patch16_224": vit_small_patch16_224, "vit_base_patch16_224": vit_base_patch16_224,
            "vit_large_patch16_224": vit_large_patch16_224, "vit_huge_patch16_224": vit_huge_patch16_224}


class ViTFc(nn.Module):
    def __init__(self, vit_name, use_bottleneck=True, bottleneck_dim=256, new_cls=False, class_num=1000):
        super(ViTFc, self).__init__()
        model_vit = vit_dict[vit_name](pretrained=True)
    ...
    def forward(self, x, return_attention=False):
        ...
        if return_attention:
            for blk in self.blocks:
                # Forward through attention module to get weights
                qkv = blk.attn.qkv(x)
                qkv = qkv.reshape(x.shape[0], x.shape[1], 3, blk.attn.num_heads, blk.attn.head_dim).permute(2, 0, 3, 1,
                                                                                                            4)
                q, k, _ = qkv[0], qkv[1], qkv[2]  # [batch, heads, seq_len, head_dim]
                attn = (q @ k.transpose(-2, -1)) * blk.attn.scale
                attn = attn.softmax(dim=-1)  # [batch, heads, seq_len, seq_len]
                attn_weights.append(attn)
                x = blk(x)  # Continue forward pass
        else:
            x = self.blocks(x)
        x = self.norm(x)
        x = x[:, 0]
    ...
    def get_cls_patch_embeddings(self, x):
        with torch.no_grad():
            x = self.patch_embed(x)
    ...
    def get_parameters(self):
        if self.new_cls:
            if self.use_bottleneck:
                parameter_list = [
                    {"params": self.blocks.parameters(), "lr_mult": 1, 'decay_mult': 2},
                    {"params": self.norm.parameters(), "lr_mult": 1, 'decay_mult': 2},
                    {"params": self.bottleneck.parameters(), "lr_mult": 10, 'decay_mult': 2},
                    {"params": self.fc.parameters(), "lr_mult": 10, 'decay_mult': 2}
                ]
            else:
